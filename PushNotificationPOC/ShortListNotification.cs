...
using PushNotificationPOC.Model;
using System;
...
namespace PushNotificationPOC
{
    public class ShortListNotification : DatabaseContext
    {
        ...
        {
            int notificationJobTitleLength = Convert.ToInt32(ConfigurationManager.AppSettings["NotificationJobTitleLength"]);

            int jobTitleLength = data.JobTitle.Length;
            string jobTitle = jobTitleLength <= notificationJobTitleLength ? data.JobTitle : (data.JobTitle.Substring(0, notificationJobTitleLength) + "...");

            string notificationBody = data.CandidateName + " has been shortlisted by " + data.NotificationCreaterUserName + " for " + data.CompanyName + "." + " Job ID " + data.JobID + " " + jobTitle;
